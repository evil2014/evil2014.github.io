...
{ 
	...
	if(this.dataObject.hotSpots[target.name].popupImage)
	{
		var _tt = this;
		var _imgP = new Image();
		_imgP.onload = function(ev)
		{
			var _image = new ImageComp();
			_image.x = _tt.dataObject.hotSpots[target.name].popupImage.x;
			_image.y = _tt.dataObject.hotSpots[target.name].popupImage.y;			
			_image.setScale(_tt.dataObject.hotSpots[target.name].popupImage.width,_tt.dataObject.hotSpots[target.name].popupImage.height);	
			_image.setImageType("");
			_image.loadImage(_tt.dataObject.hotSpots[target.name].popupImage.name);
			_tt.popupContainer.addChild(_image);
			_image.onMouseOver = function(){};
		}
		_imgP.src = this.dataObject.hotSpots[target.name].popupImage.name;
