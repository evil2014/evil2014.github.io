...
package classes.media.SyntaxChecker {
	...
	public class SyntaxChecker extends MovieClip{
		...
		{			
			...
			_fontFace = xmlLoader.mediaXML.component.@fontFace;
			_fontSize = xmlLoader.mediaXML.component.@fontSize;
		...
		private function createPromptText(obj:Object):void
		{
			...
			var txt = "";
			var styleObj ={};
			var prevCmd:DisplayObject;
			
			if(obj.wrong == 1)
			{				
				groupNo++;
				txt = xmlLoader.mediaXML.component.(@id==_compID).feedback;
				var prevCmd1 = holder.getChildByName("cmd"+(groupNo-1)+(cmdCnt-1));
				cmdCnt = 0;
				ypos = prevCmd1.y + prevCmd1.height;
			...
			{				
				...
				var prevCmd2 = holder.getChildByName("cmd"+(groupNo)+(cmdCnt-1));
				cmdCnt = 0;
				for(var i=1;i<len;i++)
				{
					groupNo++;
					...
					var gap = (i>1)?0:4;
					ypos = (prevCmd2.y + prevCmd2.height)-gap;
					styleObj = {wordWrap:true, width:_width-15};
					prevCmd2 = createTextField(ypos, txt, styleObj);
				}
				if(len > 1)
			...
			else
			{
				...
				var prevPrompt3 = (obj.showAns == 1 && prev!= null)?prev:holder.getChildByName("prompt"+(groupNo-1));
				ypos = prevPrompt3.y + prevPrompt3.height;
			}
