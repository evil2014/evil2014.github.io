package classes.media.timeline 
{
	...
	public class TimelineSlider extends MovieClip
	{
		...
		private var _parent:Timeline;
		
		public function TimelineSlider(_xml,_holder:* = null) 
		{
			this.mXmlData = _xml;
			if(_holder) this.mSlideHolder = _holder;
			this.addEventListener(Event.ADDED_TO_STAGE, addedToStageHandler, false, 0, true);
			
			var bg:Shape = new Shape();
			bg.graphics.beginFill(0xBDBBBC);
			bg.graphics.drawRect(-10, 0, 65, 340);
			bg.graphics.endFill();
			this.addChild(bg);
		}
		...
		private function createEventButtons():void
		{
			...
			for (var i:int = 0; i < mXmlData.event.length(); i++)
			{
				...
				_timelineEBtn.y = 13 + (((mEndValue - _eventValue) / mScaleValue) * mGap);
		...
		private function timelineEventChange(_btn:TimelineEventButton):void
		{
			...
			if (mCurrentEventBtn)
			{
				...
				if (this.mSlideHolder[instName]) this.mSlideHolder[instName].visible = false;
			...
			if(this.mSlideHolder[instName]) _parent.updatedEventSlide(_btn.eventId, mCurrentEventSlide);
		...
		private function createLabelAndLine()
		{
			...
			var cnt:int = 0;
			for (var i:int = mEndValue; i >= mStartValue;i-=mScaleValue )
			{	
		...
		private function addedToStageHandler(ev:Event):void
		{
			...
			this._parent = Timeline(this.parent);
			this.initiate();
