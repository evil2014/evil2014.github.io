	...
	{	
		...
		if(id == "check")
		{
			...
			for (m=0;m<dropArray.length;m++)
			{
				...
				if((dropArray[m].obj.match && dropArray[m].obj.match!="null") ){
				correctanswercount++;
				}
				}
				if(obj.matchType=="group"){
						if(dropArray[m].obj.group &&(dropArray[m].obj.group!="null" || dropArray[m].obj.group=="")){
						 if(dropArray[m].obj.empty && dropArray[m].obj.empty=="null"){	
...
/*		if(obj.grouptype=="multi"){
			...
			for (var t=0;t<groupArr[n].length;t++){
					...
					if((groupArr[n][t].dragTarget == null || groupArr[n][t].dragTarget == "null") && count < lengthArr[n]){
						totaldragTarget++;
